...
CacheGeneric<State, Addr_t> *CacheGeneric<State, Addr_t>::create(const char *section, const char *append, const char *format, ...)
{
  ...
  snprintf(size ,STR_BUF_SIZE,"%sSize" ,
      append);
...
typename CacheAssoc<State, Addr_t>::Line *CacheAssoc<State, Addr_t>::findLinePrivate(Addr_t addr, Addr_t pc)
{
  ...
  if ((*theSet)->getTag() == tag) {
    ...
    if (policy == PAR || policy == UAR) {
      ...
      if (policy == UAR) {
        (*theSet)->incnDemand();
        if (next_rrip > 0
            && pc2tracker[(*theSet)->getPC()].conf > 8
  ...
  if (policy == UAR) {
    ...
    if (pc2tracker[tmp->getPC()].conf > 8
        && (1+pc2tracker[tmp->getPC()].demand_trend) < tmp->getnDemand()
        && next_rrip > 0) {
      trackerDown4->inc();
      next_rrip/=2;
    }else{
...
*CacheAssoc<State, Addr_t>::findLine2Replace(Addr_t addr, Addr_t pc, bool prefetch)
{
  Addr_t tag    = this->calcTag(addr);
  I(tag);
  Line **theSet = &content[this->calcIndex4Tag(tag)];
  ...
  if (policy == LRUp) {
    ...
    if (policy == UAR) {
      if (pc2tracker[pc].conf > 8
          && pc2tracker[pc].demand_trend == 0) {
        default_rrip = 0;
        trackerDown1->inc();
      }else if (pc2tracker[pc].conf > 8
          && pc2tracker[pc].demand_trend == 1) {
        default_rrip /= 2;
        trackerDown2->inc();
      }else{
        trackerDown2n->inc();
...
template<class State, class Addr_t>
typename HawkCache<State, Addr_t>::Line
*HawkCache<State, Addr_t>::findLine2Replace(Addr_t addr, Addr_t pc, bool prefetch)
{
  Addr_t tag    = this->calcTag(addr);
  I(tag);
  Line **theSet = &content[this->calcIndex4Tag(tag)];
...
*CacheDM<State, Addr_t>::findLine2Replace(Addr_t addr, Addr_t pc, bool prefetch)
{
  Addr_t tag = this->calcTag(addr);
  Line *line = content[this->calcIndex4Tag(tag)];
...
*CacheDMSkew<State, Addr_t>::findLine2Replace(Addr_t addr, Addr_t pc, bool prefetch)
{ 
  Addr_t tag1 = this->calcTag(addr);
...
*CacheSHIP<State, Addr_t>::findLine2Replace(Addr_t addr, Addr_t pc, bool prefetch)
{ 
